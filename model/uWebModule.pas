...
uses
   ...
   Datasnap.DSCommonServer, Datasnap.DSHTTP, System.IOUtils;
...
begin
   PersistentClass := uUserControl.TUserControl;
end;
...
begin
   if (User = userServer) and (Password = passServer) then
      valid := True
   else
      valid := False;
end;

procedure TWebModule1.DSAuthenticationManager1UserAuthorize(Sender: TObject; EventObject: TDSAuthorizeEventObject; var valid: Boolean);
begin
   valid := True;
end;
...
begin
   try
      filePath := ExtractFilePath(ParamStr(0)) + 'serveravailable.html';

      Response.Content := TFile.ReadAllText(filePath);
      ...
      on E: Exception do
      begin
         Response.Content := 'Erro ao carregar o arquivo: ' + E.Message;
